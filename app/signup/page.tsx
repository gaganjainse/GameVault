      ...
      <Card className="w-full max-w-md relative z-10 bg-card/80 backdrop-blur-xl border-border/50">
        <CardHeader className="space-y-1 pb-4">
          ...
          <CardDescription className="text-center">
            Join the community and start building your vault
          </CardDescription>
        </CardHeader>
              ...
              <div className="grid grid-cols-2 gap-1 mt-2">
                {passwordRequirements.map((req, i) => (
                  <div key={i} className="flex items-center gap-1 text-xs">
                    <CheckCircle2 className={`h-3 w-3 ${req.test ? 'text-success' : 'text-muted-foreground'}`} />
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                if (error) {
                  toast.error('Failed to sign up with GitHub', { description: error.message })
                  setOauthLoading(null)
                }
              }}
            >
